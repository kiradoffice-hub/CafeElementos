...
function showPage(pageName) {
    const pages = ['bienvenido', 'inicio', 'tienda', 'productos', 'nosotros'];
    ...
    cartItems.innerHTML = cart.map((item, index) => `
        <div class="cart-item">
            <div class="cart-item-image">
                <img src="${item.image}" alt="${item.name}" onerror="this.style.display='none'; this.parentElement.innerHTML='<div style=\\'font-size:2rem;display:flex;align-items:center;justify-content:center;height:100%\\'>☕</div>'">
            </div>
            <div class="cart-item-info">
                <div class="cart-item-name">${item.name}</div>
                <div class="cart-item-weight">${item.weight} kg</div>
...
async function confirmOrder() {
    ...
    // Validaciones
    if (!name || !email || !phone || !address || !city || !postal || !cardNumber || !cardExpiry || !cardCVV || !cardName) {
    ...
    const itemsList = cart.map(item => 
        `• ${item.name} (${item.weight}) x${item.quantity} - $${(item.price * item.quantity).toFixed(2)} MXN`
    ).join('\n');
